...
function F = rskelf(A,x,occ,rank_or_tol,pxyfun,opts)
  ...
  for lvl = t.nlvl:-1:1
    ...
    for i = t.lvp(lvl)+1:t.lvp(lvl+1)
      ...
      n_slf = length(slf);
      n_nbr = length(nbr);
      L_ = chol(K(1:n_slf,1:n_slf,'lower');
      K(n_slf+1:n_br+n_slf,1:n_slf) = K(n_slf+1:n_br+n_slf,1:n_slf)/(L');
      K(1:n_slf,n_slf+1:n_br+n_slf) = L\K(1:n_slf,n_slf+1:n_br+n_slf);
