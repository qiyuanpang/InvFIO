...
repeat_num = 1;
delta = 10;
mus = [2^0 2^3 2^6 2^9 2^(12)];
lambdas = [2^(-16) 2^(-12) 2^(-8) 2^(-4) 2^0 2^4 2^(8) 2^(12) 2^(16)];



%dims = 2.^[12 13 14 15 16 17]
dims = 2.^[11 12]
cases = length(dims);
...
for i = 1:cases
    ...
    for j = 1:repeat_num
      if strcmp(regm, 'L1')
        [F,HODLR] = HODLR_construction( N, @(x)apply_bf(Factor, x), @(x) apply_bf_adj(Factor,x), tol_peel, fileID, occ, 1200,1200);
      elseif strcmp(regm, 'TV-L1')
        [F,HODLR] = HODLR_construction( N, @(x)apply_bf(Factor, fft(x)), @(x) N*ifft(apply_bf_adj(Factor,x)), tol_peel, fileID, occ,1200,1200);
      end
    end
    ...
    for mu = mus
      lmus = [mu/128  mu/64 mu/32 mu/16 mu mu*16 mu*32 mu*64 mu*128];
      ...
      for lambda = LAMBDAS 
        ...
        for tol=[1E-12]
            ...
            if strcmp(regm,'TV-L1')
              ...
              [x, iter] = SplitBregman(regm, mu, lambda, 1/N, @(x)N*ifft(apply_bf_adj(Factor,x)), @(x)hodlr_apply(HODLR_A, x), b, @(x)gradientxp(x, 1/N), @(x)gradientxp(x, 1/N), maxit1, maxit2, tol, @(x)RSS_inv_lu(G_A, x), f0, f0, zeros(N,1));
            elseif strcmp(regm,'L1')
              if strcmp(initf, 'sol')
            ...
            if strcmp(regm,'TV-L1')
              ...
              [x, iter] = SplitBregman(regm, mu, lambda, 1/N, @(x)N*ifft(apply_bf_adj(Factor,x)), @(x)hodlr_apply(HODLR_A, x), b, @(x)gradientxp(x, 1/N), @(x)gradientxp(x, 1/N), maxit1, maxit2, tol, @(x)hodlrqr_inv(Y_A, T_A, R_A, x), f0, f0, zeros(N,1));
            elseif strcmp(regm,'L1')
              if strcmp(initf, 'sol')
