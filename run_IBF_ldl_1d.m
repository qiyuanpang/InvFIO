...
for N= [256,1024,4096,2*8192,8*8192,32*8192]
    ...
    fileID = fopen(['results_1d/BFF_LDL_',func_name,'_N_',num2str(N),'.txt'],'w');
    ...
    RSS_apply_lu(G,f);
    t=toc;
    [e,niter] = snorm(N,@(x)(apply_bf_adj(Factor,apply_bf(Factor,x)) - RSS_apply_lu(G,x)),[],[],1);
    e = e/snorm(N,@(x)(apply_bf_adj(Factor,apply_bf(Factor,x))),[],[],1);
    ...
    [e,niter] = snorm(N,@(x)(x-apply_bf(Factor,RSS_inv_lu(G,apply_bf_adj(Factor,x)))),[],[],1);
    fprintf(fileID,'sv: %10.4e / (%4d) time %10.4e\n',e,niter,t);
