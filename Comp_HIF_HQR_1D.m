...
for i = 1:cases
    ...
    tStart=tic;
    for j = 1:repeat_num
      [F,HODLR1] = HODLR_construction( N, @(x) apply_bf(Factor,x), @(x) apply_bf_adj(Factor,x), tol_peel, fileID, occ, 64,64);
    end
    t = toc(tStart)/repeat_num;
    factime_hodlr(i) = t;
    fprintf(fileID,'time H_matrix construction 10e-6: %10.4e )\n',t);
    lvls = floor(log2(length(HODLR1))-1);
    HODLR = HODLR_transfer(HODLR1, lvls, lvls, 1); %% use another way to store HODLR
    ...
    % e = norm(apply_bf_adj(Factor,apply_bf(Factor,v)) - HODLR_apply(HODLR1,v))/norm(apply_bf_adj(Factor,apply_bf(Factor,v)));
    % fprintf('HODLR : %10.4e \n', e)
